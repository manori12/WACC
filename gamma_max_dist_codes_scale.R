...
resultsg5<-na.omit(cbind(inputs1,t(outputs)))
#resultsg10<-na.omit(cbind(inputs1,t(outputs)))
#resultsg20<-na.omit(cbind(inputs1,t(outputs)))

#-------------------ARL, SDRL for n=5----------------------------
arls<- by(resultsg5[,9], INDICES=resultsg5[,3:4], FUN=mean,  simplify=TRUE)
arlw<- by(resultsg5[,10], INDICES=resultsg5[,3:4], FUN=mean,  simplify=TRUE)
sdrls<- by(resultsg5[,9], INDICES=resultsg5[,3:4], FUN=sd,  simplify=TRUE)
sdrlw<- by(resultsg5[,10], INDICES=resultsg5[,3:4], FUN=sd,  simplify=TRUE)

#arlg5<-cbind(arls,sdrls,arlw,sdrlw)
#arlg5<-round(arlg5,2)
#saveRDS(arlg5,file="arlg5scale.RDs")
#readRDS("arlg5scale.RDs")
...
saveRDS(arlg10,file="arlg10scale.RDs")
readRDS("arlg10scale.RDs")


#-------------------ARL, SDRL for n=20----------------------------

arls <- by(resultsg20[,9], INDICES=resultsg20[,3:4], FUN=mean,  simplify=TRUE)
arlw <- by(resultsg20[,10], INDICES=resultsg20[,3:4], FUN=mean,  simplify=TRUE)
sdrls <- by(resultsg20[,9], INDICES=resultsg20[,3:4], FUN=sd,  simplify=TRUE)
sdrlw <- by(resultsg20[,10], INDICES=resultsg20[,3:4], FUN=sd,  simplify=TRUE)

arlg20<-cbind(arls,sdrls,arlw,sdrlw)
arlg20<-round(arlg20,2)
saveRDS(arlg20,file="arlg20scale.RDs")
readRDS("arlg20scale.RDs")
