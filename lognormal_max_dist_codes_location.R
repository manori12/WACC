...
#results5<-na.omit(cbind(inputs1,t(outputs)))
...
#-------------------ARL, SDRL for n=5----------------------------
arls <- by(results5[,9], INDICES=results5[,3], FUN=mean,  simplify=TRUE)
arlw <- by(results5[,10], INDICES=results5[,3], FUN=mean,  simplify=TRUE)
sdrls <- by(results5[,9], INDICES=results5[,3], FUN=sd,  simplify=TRUE)
sdrlw <- by(results5[,10], INDICES=results5[,3], FUN=sd,  simplify=TRUE)

arl5<-cbind(arls,sdrls,arlw,sdrlw)
arl5<-round(arl5,2)
saveRDS(arl5,file="arlln5lo.RDs")
readRDS("arlln5lo.RDs")


#-------------------ARL, SDRL for n=10----------------------------
arls <- by(results10[,9], INDICES=results10[,3], FUN=mean,  simplify=TRUE)
arlw <- by(results10[,10], INDICES=results10[,3], FUN=mean,  simplify=TRUE)
sdrls <- by(results10[,9], INDICES=results10[,3], FUN=sd,  simplify=TRUE)
sdrlw <- by(results10[,10], INDICES=results10[,3], FUN=sd,  simplify=TRUE)

arl10<-cbind(arls,sdrls,arlw,sdrlw)
arl10<-round(arl10,2)
saveRDS(arl10,file="arlln10lo.RDs")
readRDS("arlln10lo.RDs")


#-------------------ARL, SDRL for n=20----------------------------

arls <- by(results20[,9], INDICES=results20[,3], FUN=mean,  simplify=TRUE)
arlw <- by(results20[,10], INDICES=results20[,3], FUN=mean,  simplify=TRUE)
sdrls <- by(results20[,9], INDICES=results20[,3], FUN=sd,  simplify=TRUE)
sdrlw <- by(results20[,10], INDICES=results20[,3], FUN=sd,  simplify=TRUE)
